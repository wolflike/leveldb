 ...
 * version组成一个双向循环链表。
 * 系统文件：
 * log：顺序记录leveldb的所有操作。
 * sst：此文件就是SSTable文件。
 * CURRENT：记录当前最新的MANIFEST清单文件。
 * LOCK：leveldb的全局锁文件，在Recover和DestoryDB时用到。
 * LOG
 * MANIFEST：记录了各个阶段的VersionEdit，leveldb可通过Manifest计算还原得到最新的Version。
 */
        ...
        implements SeekingIterable<InternalKey, Slice>
{
    ...
    //下一个 manifest 文件编号，写入到 manifest 日志里
    /**
     * 该编号是全局统一的编号，生成器统一自增分配
     */
    private final AtomicLong nextFileNumber = new AtomicLong(2);

    // manifest 文件编号，每次重启后递增
    private long manifestFileNumber = 1;

    private Version current;

    //sequence 号，用于 snapshot，每次写入操作都会递增
            ...
            throws IOException
    {
        ...
        try (FileInputStream fis = new FileInputStream(new File(databaseDir, currentName));
            ...
            //将多次的versionEdit一次性全部生成一个新的version
            //通过for循环将edit全部apply到builder中，然后一次性apply到version中
            ...
            //生成最终的version
            //current+versionEdit=newVersion
            //主要是构建level0和levels
            builder.saveTo(newVersion);

            //主要设置compactionLevel、compactionScore变量
            ...
            //versionSet中的参数最终来自于manifest的最后一个edit的参数
    ...
    //选择需要合并的文件
    //getRange:用inputs获取[smallest,largest]
    //getOverlappingInputs:用[smallest,largest]获取重叠的文件
    ...
    private Compaction setupOtherInputs(int level, List<FileMetaData> levelInputs)
    {
        ...
        //到这里为止，区间[allStart,allLimit]是两层共同的最大范围，也是最终合并后的样子
