...
public class DbBenchmark
{
            ...
            throws IOException
    {
        printHeader();
        //打开一个数据库
        //从这里看出，leveldb就没有cs架构的概念，项目直接引用该库，直接在本地抽象为数据库
        open();
...
//        }
            ...
            throws IOException
    {
        ...
        //构建数据，默认1百万条,每entriesPerBatch为一个批次
        for (int i = 0; i < numEntries; i += entriesPerBatch) {
            WriteBatch batch = db.createWriteBatch();
            for (int j = 0; j < entriesPerBatch; j++) {
                //如果是顺序写,那key就是i+j,随机写key就是随机数[0,1000000]
                int k = (order == SEQUENTIAL) ? i + j : random.nextInt(num);
                byte[] key = formatNumber(k);
                //leveldb中的Slice有点东西啊
                //所有的Slice共用一个byte[] data数组，
                //它们通过length+offset来确定自己的数据
                //generator.generate(valueSize)就是截取长度为valueSize的数据
                /**
                 * 所以这里的key就是[0,1000000],占16字节
                 * value是随机生成的100字节随机数
                 */
            ...
            //开始写入这个批次（批次中可以是多个也可以是单个）
            //可以看到leveldb根本没有层次设计
            //benchmark直接使用DB接口进行数据操作
...
//        }
    ...
    private static class RandomGenerator
    {
        ...
        private RandomGenerator(double compressionRatio)
        {
            ...
            //我们一次又一次地使用有限的数据量，并确保其大于压缩窗口（32KB），
            //并且足够大，可以满足我们想要写入的所有典型值大小。
